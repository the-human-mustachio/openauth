...
### Phase 6 — Real Storage Adapters ✅ **COMPLETE**
...
- **Parameterized port-conformance suite** at `test/ports/` (`token-store.ts`, `session-store.ts`, `key-store.ts`, `config-store.ts`, `method-store.ts`, `audit-log.ts`, `fixtures.ts`, `index.ts`). Each suite is a `describe*Store({ adapterName, makeStore, … })` factory the adapter test files call. Every adapter in this phase is exercised through the same cases — single-use atomicity of `consumeCode` / `consumeRefresh` / `consumeFlow` under concurrent presentations (`Promise.all` of 3-5 calls, exactly-one-winner assertion), reuse-detection wire format (`family=…,tenant=…,subject=…`), `revokeFamily` / `revokeBySubject` propagation, TTL enforcement (`saveCode` rejects `ttl > 60_000`), encryption-at-rest verification via an adapter-supplied `inspectRawCode` callback, flow-record lifecycle (save / read / update / consume), and JWKS overlap. An adapter that fails any case is not certified per `ports/CONSISTENCY.md`.
- **Memory adapter** (existing) re-tested through the suite + memory-specific cases (`MemoryAuditLog.byKind`). The legacy `test/adapters/memory.test.ts` was rewritten so the contract tests live in the shared suite and only memory-only behaviour remains in the per-adapter file. Memory `MethodStore` shipped (`adapters/memory/method-store.ts`) so the conformance suite has a baseline for it.
- **`adapters/postgres/` (full family)** — `PostgresTokenStore`, `PostgresSessionStore`, `PostgresKeyStore`, `PostgresConfigStore`, `PostgresMethodStore`, `PostgresAuditLog`, single `INITIAL_SCHEMA_SQL` migration (idempotent `CREATE TABLE IF NOT EXISTS`), and an `executor.ts` (`PostgresExecutor` interface + `fromPostgresJs(sql)` + `fromPGlite(db)` factories). Driver: `postgres@3.4.9` (porsager/postgres) added to runtime deps; `@electric-sql/pglite@0.4.5` added to dev deps for tests. `consumeCode` / `consumeFlow` use `DELETE … RETURNING`; `consumeRefresh` uses `UPDATE … WHERE consumed_at IS NULL AND expires_at > $1 RETURNING payload` for atomic single-winner claim. Code payloads are encrypted at rest with the same `encryptPayload` / `decryptPayload` helpers the memory adapter uses (JWE A256GCM). Test: `test/adapters/postgres.test.ts` runs the full suite (41/41 cases) against PGlite — in-process WASM Postgres, no Docker.
- **`adapters/d1/` (full family)** — `D1TokenStore`, `D1SessionStore`, `D1KeyStore`, `D1ConfigStore`, `D1MethodStore`, `D1AuditLog`, `migrations.ts` (`INITIAL_SCHEMA_SQL` array of `CREATE TABLE IF NOT EXISTS` statements in SQLite dialect), `session.ts` (`primarySession(db)` + `isSessionsCapable(db)` feature-detection), and `types.ts` (`AnyD1Database`, `D1DatabaseWithSessions`, `D1SessionConstraint`, `D1DatabaseSession`). Every security-critical operation wraps its statement in `primarySession(db)`; the helper feature-detects `db.withSession("first-primary")` and falls back to plain `prepare` when Sessions is unavailable. Test: `test/adapters/d1.test.ts` against a `bun:sqlite`-backed `D1Database` shim (`test/helpers/d1-shim.ts`) — 41/41 cases, no miniflare or wrangler runtime needed.
- **`adapters/durable-object/`** — `DurableObjectSessionStore` accepts `DurableObjectStorage` directly so users wire it from inside their own DO class. `consumeFlow` and `updateFlowMethodState` use `storage.transaction(...)` for atomic read-modify-write; long-lived sessions implemented. `types.ts` pins `DurableObjectStorageLike` / `DurableObjectTransactionLike` locally so the adapter is testable without the Workers runtime. Test: `test/adapters/durable-object.test.ts` against `test/helpers/do-storage-shim.ts` — an in-process `Map` plus a `Promise`-chain-serialized transaction queue (so `Promise.all` of concurrent `consumeFlow` calls resolves to one winner). 9/9 SessionStore conformance cases pass.
- **`adapters/kv/` (read-eventual ports only)** — `KvConfigStore`, `KvMethodStore`, `KvAuditLog`. Each adapter's JSDoc states the restriction: KV is **not** acceptable for `TokenStore` / `SessionStore` per `ports/CONSISTENCY.md`. Keys: `tenant:<id>` for tenant config; `method:<tenantId>:<methodId>` for methods (with cursor-paginated `list`); `audit:<paddedTs>:<rand>` for audit events (lexicographic list returns insertion order). Test: `test/adapters/kv.test.ts` against `test/helpers/kv-shim.ts` — 13/13 cases.
- **`adapters/dynamo/` (full family)** — `DynamoTokenStore`, `DynamoSessionStore`, `DynamoKeyStore`, `DynamoConfigStore`, `DynamoMethodStore`, `DynamoAuditLog`, plus `client.ts` (a small `DynamoExecutor` interface that mirrors only the ops the adapter needs: `get` / `put` / `delete` / `consumeRefresh` / `query` / `queryByGsi`) and `sdk.ts` (`fromDynamoDBClient(docClient, tableName)` — wires the executor to `@aws-sdk/lib-dynamodb`). Single-table design — `pk` / `sk` primary key, two GSIs (`family-index` and `subject-index`) for refresh-token revocation. Every strong read uses `ConsistentRead: true`; `consumeRefresh` is a conditional `UpdateItem` with `attribute_not_exists(consumed_at) AND expires_at > :now`. Dependencies: `@aws-sdk/client-dynamodb@3.1045.0` and `@aws-sdk/lib-dynamodb@3.1045.0` added to runtime deps. Test: `test/adapters/dynamo.test.ts` against `test/helpers/dynamo-shim.ts` — an in-memory executor that faithfully implements the conditional-write + GSI semantics the adapter uses. 41/41 cases.
- **`adapters/kms/`** — `KmsKeyStore` plus `KmsBackingStore` (a pluggable interface — `putSigningKey` / `getSigningKey` / `listSigningKeys` and the encryption-key equivalents — so KMS-wrapped envelopes can be stored anywhere). Envelope encryption: private JWK material (signing) and raw 32-byte AES keys (encryption) are wrapped under a KMS master key via `KmsClientLike.encrypt`; the ciphertext is persisted in the backing store. On read, `KmsClientLike.decrypt` returns the plaintext and the adapter `importJWK`s the unwrapped private key so the framework still receives a real `CryptoKey` for JWT signing — **no surgery in `domain/jwt.ts`** is needed. `cacheUnwrappedKeys: false` opt-out for strict no-plaintext-cache deployments. `inMemoryKmsBackingStore()` is provided for dev / tests; `fromKmsClient(KMSClient, keyArn)` wires the real AWS SDK (`@aws-sdk/client-kms@3.1045.0`). Test: `test/adapters/kms.test.ts` runs the KeyStore conformance suite against a mock KMS (AES-GCM under a random master key) plus a focused envelope-encryption assertion (the wrapped JWK in the backing store contains no `"d":` or `"kty":` substrings). 7/7 cases.
- **Test infrastructure helpers** added under `test/helpers/`: `d1-shim.ts`, `do-storage-shim.ts`, `kv-shim.ts`, `dynamo-shim.ts`, `kms-mock.ts`. Each is ≤ 100 LOC and reuses the conformance suite via the parameterized factories. Total test count grew from 195 (Phase 5) to **375 across 31 files**, with the 17 OAuth 2.1 / OIDC conformance cases + 15-case provider matrix + 84 Phase-2 domain tests + Phase-3/4/5 method tests all still green.
- **Verification gates passed:**
  - `bun test` — **375/375** (Phase 5: 195 + Phase 6: 180). 17 OAuth 2.1 conformance cases unchanged.
  - `bunx tsc --noEmit -p tsconfig.json` exits 0 under `strict: true`.
  - `grep` confirms zero `hono` / `node:*` imports in `src/{types,ports,domain,adapters,methods}/`. The only `bun:sqlite` import is in `test/helpers/d1-shim.ts` (test infrastructure).

#### Phase 6 — Deferred

- **Real-network integration tests.** The PGlite / `bun:sqlite` / in-process shims faithfully implement the contracts, but only operational tests against real Postgres / D1 / DynamoDB / KMS exercise actual network behaviour. Phase 8 hardening introduces nightly jobs (private creds, ephemeral instances) that run the same conformance suite against real backends. The conformance suite is the lever — adding a new test runner is a 30-line addition once we have a backend ready.
- **Performance benchmarks.** The plan's "<100ms authorize → token round trip" gate is not enforced by the test suite — adding a benchmark harness with p99 + drift detection is Phase 8 work. Locally, PGlite runs the full 41-case conformance suite in <1s and the in-process adapters in <100ms total, so the framework overhead is well under budget; what's missing is a real-backend baseline.
- **Health checks (`adapter.ping()`).** Listed in the plan's acceptance criteria. The Postgres / D1 / DynamoDB adapters can implement `ping()` via a trivial `SELECT 1` / `prepare("SELECT 1").run()` / `GetItem`, but the IdP runtime doesn't call them anywhere today. Land when Phase 7 / Phase 8 introduces a `/health` endpoint that needs them.
- **Deployment runbooks.** The DoD calls for "Cloudflare, AWS Lambda, Node + Postgres" runbooks. The adapter JSDoc shows the wiring snippets; full runbooks (Terraform / Wrangler templates, secrets management, observability) land alongside Phase 7 (which also dictates the IaC story).
- **DynamoDB Time-to-Live attribute.** The adapter writes a `ttl` attribute (seconds) on every token / code / flow item so operators can enable native TTL on the table. We do not require it — the framework's own clock-driven expiry checks remain the authoritative path. Documented in the adapter index JSDoc.
- **Postgres `updateFlowMethodState` race.** The DynamoDB session-store JSDoc notes a small race window between `get` and `put` in `updateFlowMethodState`; mitigated in practice because the framework never calls update concurrently with consume on the same flowId, but tightening this to a single conditional UPDATE (DynamoDB Transactions or a `ConditionExpression: attribute_exists(pk)` extension to the executor) is a Phase 8 polish item.
- **KV adapter integration with the framework `MethodCache`.** `MethodCache.onInvalidate` would let `ConfigStore`-triggered tenant config updates also invalidate the in-process method cache when KV is the backing store; the wire-up is one line but waits for Phase 7's console mutations to land.

#### Phase 6 — Decisions captured for later phases

- **Per-port consistency strategy chosen per adapter.** D1 uses `withSession("first-primary")` on the security-critical paths and plain `prepare` on read-eventual paths. DynamoDB uses `ConsistentRead: true` on the security-critical paths and the default eventual read on `ConfigStore` / `MethodStore` / `AuditLog`. Postgres uses the natural row-level consistency of SQL with `DELETE … RETURNING` / `UPDATE … RETURNING`. KV is restricted to the read-eventual ports entirely. This per-port strategy is captured in the adapter JSDoc and is the reference for future adapters (e.g. Spanner, FoundationDB) — start by mapping each port to one of these four idioms.
- **Encryption-at-rest is an adapter responsibility, not a domain responsibility.** The encryption / decryption helpers (`domain/crypto.encryptPayload` / `decryptPayload`) are reused by every `TokenStore` adapter; the contract is "ciphertext at rest" not "raw payload in the DB". Adapters that delegate to a managed encryption service (KMS, Vault) still satisfy the contract by wrapping the payload before write — the conformance suite verifies the wire format via `inspectRawCode`.
- **KMS adapter is opt-in envelope encryption for keys, not the only `KeyStore` flavour.** `KmsKeyStore` exists for deployments that require KMS-managed signing key custody; the Postgres / D1 / DynamoDB / memory `KeyStore`s remain valid for deployments that don't. The framework does not assume any particular `KeyStore` provenance — `signingKey.privateKeyRef` is always a `CryptoKey` by the time `signAccessToken` sees it. Phase 8 may add a custom-signer hook for direct KMS Sign if a customer requires no-plaintext-key handling; until then the cache-on-decrypt path is the documented pattern.
- **`fromPostgresJs` / `fromPGlite` / `fromDynamoDBClient` / `fromKmsClient` are the SDK boundary.** Production users call one factory per adapter; the adapter never imports the SDK directly. This keeps the `@_mustachio/openauth` package importable in environments that don't ship the AWS SDK (Workers, edge) and keeps the adapter's `peerDependencies` story clean — `@aws-sdk/*` are runtime deps only because the user-facing `from*` factories need them; future tree-shaking work can move them to optional deps once the bundler story is more mature.
- **Test shims live next to test suites, not in `src/`.** `test/helpers/d1-shim.ts`, `do-storage-shim.ts`, `kv-shim.ts`, `dynamo-shim.ts`, `kms-mock.ts` are all infrastructure for the conformance suite. They MUST NOT be re-exported from the package — they're not certified production-grade and shipping them would muddle the consistency story. The conformance suite is the contract; the shims are how we exercise it.
- **Adapter clocks are injectable, uniformly.** Every adapter accepts `clock?: () => number` so deterministic tests advance time. Phase 6 uses this in all conformance suites; Phase 7 / Phase 8 chaos tests should rely on the same option.
- **`MethodStore` is now a first-class port across every adapter.** Phase 1 deferred a memory `MethodStore`; Phase 6 added one (and Postgres / D1 / DynamoDB / KV equivalents). The framework can now route method-config writes through `MethodStore` independently of `ConfigStore` — the console (Phase 7) can use this to give operators a `PUT /admin/tenants/:id/methods/:methodId` endpoint without rewriting the whole tenant config.

##### Open items surfaced during Phase 6

- The DynamoDB `updateFlowMethodState` non-atomic read-modify-write should ideally be a single conditional `UpdateItem` with `attribute_exists(pk)`. The current `DynamoExecutor.put` interface accepts only the `not-exists` condition; extending it to support `exists` is a four-line addition. Deferred — the framework's call ordering keeps this safe in practice.
- The PostgresJs driver returns `bytea` columns hex-encoded by default (`\x...`); the adapter handles that, plus `Uint8Array`, plus `Buffer.from({type, data})`. If future drivers surface yet another representation, the `ensureBytes` helper in `key-store.ts` is the one place to extend.
- Conformance test wall-clock: the full suite (all eight adapters) now runs in ~1.5s. If the matrix grows by another factor (Phase 8 adds DPoP / PAR cases), we may want to parallelize per-adapter test files via Bun's test runner config — currently they share state through `beforeAll` per file.
