...
- 74b5efe: Add `IdPOptions.subjectKey`, so `sub` can be derived from an identifier the host controls.
...
- 6cdf008: Add `IdPOptions.onTokenIssued`, exposing the derived subject id to the host.
...
- e026e4d: Close seven confirmed post-rebuild findings. Several are breaking; each is listed below with what you need to change.
...
- 1ccaf97: Fix every URL the library emits when the IdP is mounted under a path prefix.
...
- f48888b: Correct the 0.13.1 release note, and stop the SAML redirect-binding SLO tests failing on Bun.
