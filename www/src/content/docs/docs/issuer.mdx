...
You can also provide a function that returns the providers dynamically. This is useful for
multi-tenant applications where you want to load provider configurations from a database
based on the request (e.g., subdomain).
...
const app = issuer({
  providers: async (ctx) => {
    ...
    const workspace = await db.workspaces.findUnique({ where: { domain: host } })

    return {
      github: GithubProvider({
        clientID: workspace.githubClientID,
        clientSecret: workspace.githubClientSecret
      })
    }
  },
})
...
For environments where you can't use subdomains or headers to identify tenants (e.g., AWS Lambda URLs),
OpenAuth provides explicit tenant routes that encode the tenant ID in the URL path.

Use `/tenant/:tenantId/authorize` instead of `/authorize`:

```ts title="issuer.ts"
const app = issuer({
  providers: async (ctx) => {
    // Get tenant ID from the URL path (set automatically for tenant routes)
  ...
  success: async (ctx, value, req) => {
    // tenantId is available in the success callback
    if (value.tenantId) {
      console.log("Tenant:", value.tenantId)
    }
    return ctx.subject("user", {
      userID: value.email,
      tenantId: value.tenantId, // Store in subject for refresh access
    })
  },
})
```

The tenant routes are:
- `/tenant/:tenantId/authorize` - Entry point for tenant-specific auth
- `/tenant/:tenantId/:provider/authorize` - Provider authorization
- `/tenant/:tenantId/:provider/callback` - Provider callback

This ensures the tenant context survives OAuth redirects, even when the callback URL can't vary per tenant.

:::caution[Tenant ID Format]
Tenant IDs must be alphanumeric with underscores and hyphens only, max 64 characters.
Valid: `acme`, `tenant-123`, `TENANT_456`
Invalid: `../admin`, `tenant/evil`, `<script>`
:::
...
<Segment>
<Section type="parameters">
- <p>[<code class="key">providers</code>](#issuerinput.providers) <code class="primitive">Record&lt;string, Provider&gt; | ((ctx: Context) => Promise&lt;Record&lt;string, Provider&gt;&gt;)</code></p>
...
<Section type="parameters">
<InlineSection>
**Type** <code class="primitive">Record&lt;string, Provider&gt; | ((ctx: Context) => Promise&lt;Record&lt;string, Provider&gt;&gt;)</code>
</InlineSection>
</Section>
The providers that you want your OpenAuth server to support. 

Can be a static object:
...
Or an async function that returns the providers based on the request context:
